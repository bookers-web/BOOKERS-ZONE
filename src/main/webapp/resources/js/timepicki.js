...
(function($) {
	...
	$.fn.timepicki = function(options) {
		...
		var defaults = {
			format_output: function(tim, mini, sec, meri) {
			    if (settings.show_meridian) {
			        ...
			        sec = Math.min(Math.max(parseInt(sec), 0), 59);
			        if (sec < 10)
			            sec = "0" + sec;

					return tim + ":" + mini + ":"  + sec + " " + meri;
			    } else {
		...
		return this.each(function() {
			...
			var new_ele = $(
				"<div class='timepicker_wrap " + settings.custom_classes + "'>" +
					...
					"<div class='sec'>" +
						top_arrow_button +
						"<div class='se_tx'><input type='text' class='timepicki-input'" + (settings.disable_keyboard_mobile ? "readonly" : "") + "></div>" +
						bottom_arrow_button +
					"</div>"
					);
			if(settings.show_meridian){
			...
			$(".timepicki-input").keydown(function (keyevent) {
			    ...
			    function validate() {
			        ...
			        } else if (grandfatherDiv.hasClass("sec")) { /// SECONDS


			            if (isValidNumber) {

			                var seconds = Math.sec(Math.max(parseInt(input.val()), 0), 59);
			...
			inputs.on('keydown', function(e) {
				...
				} else if (input.closest('.timepicker_wrap .sec').length) {
					change_sec(null, direction);
			...
			function set_value(event, close) {
				...
				var sec = ele_next.find(".se_tx input").val();
				var meri = "";
				if(settings.show_meridian){
				...
				if (tim.length !== 0 && mini.length !== 0 && (!settings.show_meridian || meri.length !== 0)) {
					...
					ele.attr('data-timepicki-sec', sec);

					if(settings.show_meridian){
						ele.attr('data-timepicki-meri', meri);
						// set the formatted value
						ele.val(settings.format_output(tim, mini, sec, meri));
					}else{
						ele.val(settings.format_output(tim, mini, sec));
					}
				}
			...
			function set_date(start_time) {
				...
				if (ele.is('[data-timepicki-tim]')) {
					...
					se = Number(ele.attr('data-timepicki-sec'));
					if(settings.show_meridian){
				...
				if (se < 10) {
					ele_next.find(".se_tx input").val("0" + se);
				} else {
					ele_next.find(".se_tx input").val(se);
				}
				if(settings.show_meridian){
					if (mer < 10) {
			...
			function change_sec(cur_ele, direction) {
				var cur_cli = "sec";
				var cur_sec = Number(ele_next.find("." + cur_cli + " .se_tx input").val());
				var ele_st = 0;
				var ele_en = 59;
				var step_size = Number(settings.step_size_seconds);
				if ((cur_ele && cur_ele.hasClass('action-next')) || direction === 'next') {
					if (cur_mins + step_size > ele_en) {
						ele_next.find("." + cur_cli + " .se_tx input").val("00");
						if(settings.overflow_seconds){
							change_time(null, 'next');
						}
					} else {
						cur_sec = cur_sec + step_size;
						if (cur_sec < 10) {
							ele_next.find("." + cur_cli + " .se_tx input").val("0" + cur_sec);
						} else {
							ele_next.find("." + cur_cli + " .se_tx input").val(cur_sec);
						}
					}
				} else if ((cur_ele && cur_ele.hasClass('action-prev')) || direction === 'prev') {
					if (cur_sec - step_size <= -1) {
						ele_next.find("." + cur_cli + " .se_tx input").val(ele_en + 1 - step_size);
						if(settings.overflow_seconds){
							change_time(null, 'prev');
						}
					} else {
			...
			$(cur_prev).add(cur_next).on("click", function() {
				...
				if (cur_ele.parent().attr("class") == "time") {
					...
					change_mins(cur_ele);
				} else if (cur_ele.parent().attr("class") == "sec") {
					change_sec(cur_ele);
				} else {
					if(settings.show_meridian){
						change_meri(cur_ele);
